...
public final class GetAnimes extends JavaPlugin {
    ...
    private GetAnimes() {
        super(new JvmPluginDescriptionBuilder("io.githuhb.usfese.getanimes", "0.2.1")
                .name("Get Animes")
                .info("从Bangumi获取今日新番")
                .author("usfese")
                .build());
    ...
    public void onEnable() {
        ...
        Listener listener= GlobalEventChannel.INSTANCE.subscribeAlways(MessageEvent.class, event->{
            ...
            if (Pattern.matches("^查询 \\d{1,}$", event.getMessage().contentToString())) AnimeGetter.sendInfo(event);
        });
    }
}